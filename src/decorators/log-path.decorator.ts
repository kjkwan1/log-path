 ...
 * 
 * @param options - Configuration options for the LogPath decorator
...
export const LogPath = (options: LogPathOptions = {}) => {
    return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
        ...
        descriptor.value = (...args: any[]) => {
            ...
            if (options.isTopLevel || !contexts.has(this)) {
                const processId2 = uuidv4();
                context = {
                    processId: processId2,
                    parentId: contexts.get(this)?.processId
                };
            ...
            if (result instanceof Promise) {
                result
                    .catch((e) => logError(config, context, target.constructor.name, propertyKey, e))
                    .finally(() => {
...
) => `[${logLevel} processID ${processId}]: ${action} ${className}.${methodName}`;
...
const logEntry = (
    ...
    const logData: LogData = {
      ...
      action: 'Enter',
      timestamp: Date.now(),
      message: message(logLevel, context.processId, className, methodName, 'Enter')
    };
  ...
  const logError = (
    ...
    const logData: LogData = {
      ...
      timestamp: Date.now(),
      message: message('error', context.processId, className, methodName, 'Enter')
    };
...
const sendLog = (config: LogConfig, logData: LogData, logLevel: string, args: any[] = []) => {
    if (config.devMode) {
        console.log(logData.message += ` with args ${JSON.stringify(args)}`);
        return;
    }
